...
public class StringEQ_01 {
	...
	public static void main(String[] args) {
		...
		// 같은지 비교
		// 정수형과 실수형을 같이 계산하면 정수형이 실수형으로 변경되어 출력
		...
		// 문자열 변수를 일반(Primirive) 변수처럼 선언하면
		// == 연산으로 비교가 가능(된다x )하다
		String str11 = "KOREA";
		...
		/*
		 * 문자열(변수)은 생성하는 방법에 따라 일반 변수와 다른 현상을 보일때가 있다. 특히 변수에 저장된 값이 같음에도 불구하고 EQ(==)
		 * 연산을 사용하여 같은지 비교하면 false 결과가 나기도 한다. 만약 코드 중간에 어떤 이유로 이러한 현상이 발생한다면 if(문자열변수
		 * == "문자열") 과 같은 코드에서 예측 할 수없는 논리적 오류가 발생 할 것이다.
		 * 
