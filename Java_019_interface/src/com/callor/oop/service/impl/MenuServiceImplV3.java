...
public class MenuServiceImplV3 implements MenuService {
	...
	/*
	 * 메뉴를 보여주고
	 * 선택된 메뉴에 따라 카트에 추가, 삭제 하거나 리스트를 보여줘야한다.
	 * 이 기능을 MenuService에서 구현하는 것이 아니라
	 * selectMenu()를 호출한 곳에서 구현을 해야 한다.
	 * 
	 * 호출한 곳에서 selectMenu() 결과에 따라 기능을 수행하려면
	 * 사용자가 메뉴를 보고 선택을 했을때 선택된 결과값을 호출한 곳에 return 해줘야 한다.
	 * 
	 * 이렇게 설계를 변경하려면 method를 void type이 아닌 int나 String 등 다른 type으로 변경해야한다. 클래스
	 * method를 변경하려면 interface 부터 변경하고, interface를 impl... 한 모든 클래스를 다 변경해야 한다.
	 * 
	...
	/*
	 * 메뉴를 확인한 후 사용자가 메뉴를 선택하면 'menuItem' 에 적절한 값을 담고 return을 수행하도록 코드 구현했다.
	 * 
	 * QUIT 선택하면 menuItem = null 1 ~ 3 중에서 선택하면 menuItem = 선택한 숫자
	 * 
	 */
