...
/*
 * CartServiceImplV1 클래스에서
 * extends (확장), 상속 받아 v2 생성
 */
public class CartServiceImplV2 extends CartServiceImplV1 implements CartServiceV2 {

	/*
	 * CartServiceImplV2 는 인스턴스 객체와 생성자만 있는 상태
	 */
	// private List<CartVO> cartList;
	private final Scanner scan; // 스캐너라는 클래스를 이용해서 스캔을 선언만
	private String cartFileName; //스트링 타입의 변수 선언만

	public CartServiceImplV2() {
		scan = new Scanner(System.in);
		cartList = new ArrayList<CartVO>();
		cartFileName = "src/com/callor/shop/cartList.csv"; // 1번 파일이름 결정
	}
	...
	public void saveCartToFile() {
		 ...
		 *  따라서 FileWriter 객체를 생성할때는 반드시
		 *  try...catch로 묶어야한다.
		 *  
		 *  결국 이 코드는 try문 안에서 만들어진다.
		 *  
		 */
		try {
			// 반드시 try catch로 묶어야 코드가 오류가 안남
